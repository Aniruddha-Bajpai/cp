...
public class Main {
    ...
    /*
        Random stuff to try when stuck :
            - if its 2C then its dp
            - for combo/probability problems,expand the given form we're interested in
            - make everthing the same then build an answer (constructive,make everthing 0 then do something)
            - something appears in parts of 2 ---> model as graph
            - assume a greedy and then try to show why it works
            - find way to simplify into one variable if multiple exists
            - treat it like fnc ( note any passing thoughts / algo that could be used so you revisit them )
            - find lower and upper bounds of answer
            ...
            - instead of solving for answers ,try soving for complement(ex, find n-(min) instead of max)
            - draw something
            - simulate a process
         ...
         Random stuff to check when WA :
            ...
            - switced variables
            - wrong MOD
            - hardcoded ege case incorrectly
         ...
         Random stuff to check when RTE :
            ...
            - division by 0;
            - edge case for empty lists / datastructure / N = 1
     */
    ...
    public static void sort(int[] arr){
        // because Arrays.sort() uses quicksort O(Nlog(N)) which dumb as it takes O(n2) in scencarios
    ...
    public static void push(TreeMap<Integer,Integer>nap,int k,int v){
        //map[k]+=v;
        if(!nap.containsKey(k))
            nap.put(k,v);
        else
            nap.put(k,nap.get(k)+v);
    }
    public static void pull(TreeMap<Integer,Integer>nap ,int k,int v){
        //assumes nap[k] >=v
        // nap[k] -= v
        int lol = nap.get(k);
        if(lol==v)
...
class RangeBit
{
    //Template.FenwickTree and Template.RangeBit are faster than Template.LazySegTree by constant factor
    final int[] value;
